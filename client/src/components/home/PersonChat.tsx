import React from "react";
import { useDispatch, useSelector } from "react-redux";
import { SET_ACTIVE } from "../../redux/chatReducer";
import { RootState } from "../../redux/store";
import theme from "../../utils/theme";
import { ChatInfo, PersonChatStyled } from "../styled/Home.styled";

export default function PersonChat({
	name,
	date,
	message,
	index,
}: {
	name: string | undefined;
	date: string | undefined;
	message: string | undefined;
	index: number;
}) {
	const dispatch = useDispatch();
	const active = useSelector((state: RootState) => state.chats.active);

	const handleChange = () => {
		dispatch(SET_ACTIVE(index));
	};
	/*			className="single-chat cursor-hover"
			id={index === active ? "highlight" : ""}
			onClick={handleChange}
			*/

	return (
		<PersonChatStyled
			style={{
				backgroundColor:
					index === active ? theme.colors.lightBlue : "transparent",
			}}
			onClick={handleChange}
		>
			<img
				className="home-user-img"
				alt="no"
				src="data:image/png;base64,"
			/>
			<ChatInfo>
				<div className="group-name">
					<h3>{name && name}</h3>
				</div>
				<p style={{ fontSize: "13px", marginTop: "3px", opacity: "0.7" }}>
					{message ? message : "Say Hi!"}
				</p>
			</ChatInfo>
		</PersonChatStyled>
	);
}
